...
extension UIView {
     ...
     */
    @objc(ad_constrainToSize:withPriority:) public func ad_constrain(to size: CGSize, with priority: UILayoutPriority) {
        translatesAutoresizingMaskIntoConstraints = false
        let constraints = [
            NSLayoutConstraint(
                item: self,
    ...
    @objc(ad_constrainToSize:) public func ad_constrain(to size: CGSize) {
        ad_constrain(to: size, with: UILayoutPriority.required)
    }
